...
void SceneNode::Update(glm::mat4 projectionMatrix, Camera* camera) {
  if (m_object != nullptr) {
    // if a parent node exists, get parent's raw matrix for calculations
    if (m_parent) {
      m_worldTransform =
          m_parent->GetWorldTransform() * this->GetLocalTransform();
    } else {  // this is the root, world and local transform are the same
    ...
    m_shader->SetUniformMatrix4fv("model",
                                  &m_worldTransform.GetInternalMatrix()[0][0]);
    m_shader->SetUniformMatrix4fv("view",
...
// Returns the world  transform stored in our SceneNode
// which can then be modified
