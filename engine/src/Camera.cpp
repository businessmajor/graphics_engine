...
void Camera::MouseLook(int mouseX, int mouseY) {
  ...
  float deltaX = -1.0f * (newMousePosition.x - m_oldMousePosition.x);

  // Use the difference to adjust the yaw of the camera
  float yaw =
      deltaX * 0.005f;  // 0.005f is a sensitivity factor, adjust as needed

  // Rotate the m_viewDirection around the m_upVector
  m_viewDirection = glm::rotate(m_viewDirection, yaw, m_upVector);

  // Normalize the m_viewDirection
...
void Camera::MoveLeft(float speed) {
  ...
  // Move the camera to the left along the right vector
  m_eyePosition += speed * rightVector;
}

void Camera::MoveRight(float speed) {
  glm::vec3 rightVector = glm::cross(m_upVector, m_viewDirection);
  // Move the camera to the right along the right vector
  m_eyePosition -= speed * rightVector;
}
...
glm::mat4 Camera::GetWorldToViewmatrix() const {
  ...
  // setup as it is.
  return glm::lookAt(m_eyePosition, m_eyePosition + m_viewDirection,
                     m_upVector);
}
